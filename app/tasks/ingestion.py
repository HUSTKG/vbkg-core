    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if auth_params:
                ...
                if auth_type == "basic":
                    client.auth = (auth_params.get("username", ""), auth_params.get("password", ""))
                
                elif auth_type == "bearer":
                    request_headers["Authorization"] = f"Bearer {auth_params.get('token', '')}"
            ...
            # Make the request
            response = await client.get(api_url, headers=request_headers, params=request_params)
