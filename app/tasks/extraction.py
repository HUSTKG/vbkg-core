...
async def process_file(file_id: UUID) -> bool:
    ...
    try:
        ...
        await supabase.table("file_uploads").update({"upload_status": "processing"}).eq(
            "id", str(file_id)
        ).execute()
        ...
        for i, chunk in enumerate(chunks):
            ...
            # Log progress
            logger.info(
                f"Extracted {len(entities)} entities from chunk {i+1}/{len(chunks)}"
            )
            ...
            # Log progress
            logger.info(
                f"Extracted {len(relationships)} relationships from chunk {i+1}/{len(chunks)}"
            )
        ...
        logger.info(
            f"Successfully processed file {file_id}: {total_entities} entities, {total_relationships} relationships"
        )
        return True
        ...
        try:
            ...
            logger.error(
                f"Error updating file status for {file_id}: {str(update_error)}"
            )
...
async def save_extraction_results(
    ...
    try:
        ...
        logger.error(
            f"Error saving extraction results for file {file_id}, chunk {chunk_index}: {str(e)}"
        )
        return False
...
async def sync_extraction_results_to_graph(extraction_result_id: UUID) -> bool:
    ...
    try:
        ...
        response = (
            ...
            .select("*")
            .eq("id", str(extraction_result_id))
            .execute()
        )
        ...
        for entity in entities:
            ...
            existing_response = (
                ...
                .select("id")
                .eq("entity_text", entity["entity_text"])
            ...
            if existing_response.data:
                ...
                create_response = (
                    await supabase.table("kg_entities").insert(entity_data).execute()
                )
        ...
        await supabase.table("extraction_results").update(
            {"synced_to_graph": True, "synced_at": datetime.now().isoformat()}
        ...
        logger.error(
            f"Error syncing extraction result {extraction_result_id} to graph: {str(e)}"
        )
        return False
