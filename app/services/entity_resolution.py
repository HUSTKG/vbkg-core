...
from app.nlp.embeddings import calculate_similarity, get_entity_embedding
from app.schemas.entity import Entity, EntityConflict, EntityResolution
...
class EntityResolutionService:
    ...
    def _has_property_conflicts(
        ...
        for prop in common_props:
            ...
            if (
                self._calculate_text_similarity(str(props1[prop]), str(props2[prop]))
                < 0.7
            ):
                return True
    ...
    async def create_entity_conflict(
        ...
        try:
            ...
            existing_response = (
                await supabase.table("entity_conflicts")
                .select("*")
            ...
            response = (
                await supabase.table("entity_conflicts").insert(conflict_data).execute()
            )
            if response.data:
                logger.info(
                    f"Created entity conflict between {entity_id_1} and {entity_id_2} with score {similarity_score}"
                )
    ...
    async def resolve_entity_conflict(
        self, conflict_id: UUID, resolution: EntityResolution, resolved_by: UUID
    ) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if resolution.resolution_type == "merge":
                ...
                merged_entity = await self.merge_entities(
                    ...
                    keep_id=(
                        resolution.keep_entity_id if resolution.keep_entity_id else None
                    ),
                ...
                resolution_result = {
                    ...
                    or "Manually determined to be separate entities",
                }
                resolution_status = "resolved"
                ...
                entity_to_keep = (
                    entity2["id"]
                    if resolution.keep_entity_id == entity1["id"]
                    else entity1["id"]
                )
                entity_to_delete = (
                    entity1["id"] if entity_to_keep == entity2["id"] else entity2["id"]
                )
            ...
            if response.data:
                logger.info(
                    f"Resolved entity conflict {conflict_id} with action {resolution.resolution_type}"
                )
                return response.data[0]
    ...
    async def merge_entities(
        ...
        try:
            ...
            entity1_response = (
                await supabase.table("kg_entities")
                .select("*")
                .eq("id", str(entity1_id))
                .execute()
            )
            entity2_response = (
                await supabase.table("kg_entities")
                .select("*")
                .eq("id", str(entity2_id))
                .execute()
            )

            if not entity1_response.data or not entity2_response.data:
                logger.warning(
                    f"One or both entities not found: {entity1_id}, {entity2_id}"
                )
                return None
            ...
            async with asyncio.Lock():  # Ensure atomicity of the merge operation
                ...
                if create_new:
                    new_entity_data = {
                        ...
                        "source_document_id": entity1.get("source_document_id")
                        or entity2.get("source_document_id"),
                    }
                    ...
                    new_entity_response = (
                        await supabase.table("kg_entities")
                        .insert(new_entity_data)
                        .execute()
                    )
                    if not new_entity_response.data:
                        logger.error(
                            f"Failed to create new entity during merge of {entity1_id} and {entity2_id}"
                        )
                        return None
                    ...
                    target_entity = (
                        entity1 if str(target_entity_id) == entity1["id"] else entity2
                    )
                    ...
                    update_data = {
                        ...
                        "verified_by": (
                            str(resolved_by)
                            if resolved_by
                            else target_entity.get("verified_by")
                        ),
                        "verification_notes": target_entity.get(
                            "verification_notes", ""
                        )
                        + " | Updated by entity merge",
                    }
                    ...
                    if not update_response.data:
                        logger.error(
                            f"Failed to update entity {target_entity_id} during merge"
                        )
                        return None
                ...
                if str(entity1_id) != str(target_entity_id):
                    await supabase.table("kg_entities").delete().eq(
                        "id", str(entity1_id)
                    ).execute()
            ...
            # Log the merge
            logger.info(
                f"Merged entities {entity1_id} and {entity2_id} into {target_entity_id}"
            )
    ...
    def _merge_entity_properties(
        ...
        for key in set(props1.keys()).intersection(set(props2.keys())):
            if entity1.get("is_verified", False) and not entity2.get(
                "is_verified", False
            ):
                merged_props[key] = props1[key]
            elif entity2.get("is_verified", False) and not entity1.get(
                "is_verified", False
            ):
        ...
        return {"entity_text": entity_text, "properties": merged_props}

    async def _update_entity_relationships(
        self, source_id1: UUID, source_id2: UUID, target_id: UUID | None
    ) -> None:
        """
        ...
        try:
            ...
            ).eq("source_entity_id", str(source_id1)).execute()

            await supabase.table("kg_relationships").update(
                {"source_entity_id": str(target_id)}
            ).eq("source_entity_id", str(source_id2)).execute()

            # Update relationships where the entities are the target
            await supabase.table("kg_relationships").update(
                {"target_entity_id": str(target_id)}
            ).eq("target_entity_id", str(source_id1)).execute()

            await supabase.table("kg_relationships").update(
                {"target_entity_id": str(target_id)}
            ).eq("target_entity_id", str(source_id2)).execute()
            ...
            if duplicate_ids:
                for dup_id in duplicate_ids:
                    await supabase.table("kg_relationships").delete().eq(
                        "id", dup_id
                    ).execute()

                logger.info(
                    f"Deleted {len(duplicate_ids)} duplicate relationships after merging entities"
                )
    ...
    async def batch_detect_conflicts(
        ...
        try:
            ...
            stats = {"entities_processed": 0, "conflicts_detected": 0, "errors": 0}
            ...
            for entity in entities:
                try:
                    ...
                    logger.error(
                        f"Error processing entity {entity['id']} in batch: {e}"
                    )
                    stats["errors"] += 1
                    stats["entities_processed"] += 1
    ...
    async def get_entity_conflicts(
        ...
        try:
            supabase = await get_supabase()
            # Build the query
            query = supabase.table("entity_conflicts").select(
            ...
            if entity_id:
                ...
                query = query.or_(
                    f"entity_id_1.eq.{entity_id_str},entity_id_2.eq.{entity_id_str}"
                )
    ...
    async def get_conflict_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_count = (
                total_response.count if hasattr(total_response, "count") else 0
            )
            ...
            pending_count = (
                pending_response.count if hasattr(pending_response, "count") else 0
            )
            ...
            resolved_count = (
                resolved_response.count if hasattr(resolved_response, "count") else 0
            )
            ...
            ignored_count = (
                ignored_response.count if hasattr(ignored_response, "count") else 0
            )

            # Conflicts by type
            type_response = await supabase.rpc("count_conflicts_by_type", {}).execute()
            ...
            return {
                ...
                "resolution_rate": (
                    (resolved_count / total_count) if total_count > 0 else 0
                ),
            }
...
async def find_entity_conflicts(
    ...
    return await entity_resolution_service.find_entity_conflicts(
        entity_id=entity_id, text_threshold=threshold, max_conflicts=max_conflicts
    )
...
async def resolve_entity_conflict(
    conflict_id: UUID, resolution: EntityResolution, resolved_by: UUID
) -> Optional[Dict[str, Any]]:
    """
    Resolve an entity conflict.
    """
    return await entity_resolution_service.resolve_entity_conflict(
        conflict_id=conflict_id, resolution=resolution, resolved_by=resolved_by
    )
