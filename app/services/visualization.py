...
class VisualizationService:
    ...
    async def create_visualization(
        self, visualization_in: VisualizationCreate, user_id: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                not visualization_in.query_id
                and not visualization_in.cypher_query
                and not visualization_in.entity_id
            ):
                raise HTTPException(
            ...
            await self.user_service._log_user_activity(
                ...
                details={
                    "visualization_id": response.data[0]["id"],
    ...
    async def get_visualization(self, visualization_id: str) -> Dict[str, Any]:
        ...
        try:
            supabase = await get_supabase()
            response = (
                await supabase.from_("visualizations")
                .select("*")
    ...
    async def update_visualization(
        self, visualization_id: str, visualization_in: VisualizationUpdate
    ) -> Dict[str, Any]:
    ...
    async def delete_visualization(self, visualization_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            await supabase.from_("visualizations").delete().eq(
                "id", visualization_id
            ).execute()

            await self.user_service._log_user_activity(
                user_id=visualization_id,
    ...
    async def get_visualization_data(
        self, visualization_id: str, params: Optional[Dict[str, Any]] = None
    ) -> VisualizationData:
        ...
        try:
            ...
            if visualization_type == VisualizationType.GRAPH:
                nodes, edges = self._transform_data_for_graph(
                    data, visualization["config"]
                )
                result.nodes = nodes
                result.edges = edges
            elif visualization_type == VisualizationType.TREE:
                nodes, edges = self._transform_data_for_tree(
                    data, visualization["config"]
                )
                result.nodes = nodes
                result.edges = edges
            ...
            await self.user_service._log_user_activity(
                user_id=visualization["created_by"],
                action="view_visualization_data",
                resource_id=visualization_id,
                resource_type="visualization",
                details={
                    "visualization_id": visualization_id,
        ...
        try:
            ...
            for record in data:
                ...
                if source and target and rel:
                    edge = self._create_edge_object(
                        rel, source.get("id"), target.get("id"), edge_config
                    )
                    edges.append(edge)
    ...
    def _create_node_object(
        ...
        node_id = node.get(
            "id",
            str(node["identity"]) if hasattr(node, "identity") else str(uuid.uuid4()),
        )
        ...
        if size_field and size_field in node_props:
            ...
            try:
                ...
                size = self._map_value_to_range(
                    field_value, 0, 100, size_range[0], size_range[1]
                )
    ...
    def _create_edge_object(
        ...
        rel_id = rel.get(
            "id",
            str(rel["identity"]) if hasattr(rel, "identity") else str(uuid.uuid4()),
        )
        ...
        if width_field and width_field in rel_props:
            ...
            try:
                ...
                width = self._map_value_to_range(
                    field_value, 0, 100, width_range[0], width_range[1]
                )
            except (ValueError, TypeError):
                pass
        ...
        try:
            ...
            if chart_type in ["bar", "line", "pie"]:
                ...
                for record in data:
                    ...
                    y_value = (
                        self._extract_value_from_record(record, y_field)
                        if y_field
                        else 1
                    )

                    group = (
                        self._extract_value_from_record(record, group_by_field)
                        if group_by_field
                        else "default"
                    )
                    ...
                    if key not in grouped_data:
                        grouped_data[key] = {"x": x_value, "y": 0, "group": group}
            ...
            # For scatter plots, we need individual points
                ...
                for record in data:
                    ...
                    z_value = (
                        self._extract_value_from_record(record, z_field)
                        if z_field
                        else 0
                    )
                    color_value = (
                        self._extract_value_from_record(record, color_field)
                        if color_field
                        else None
                    )
                    size_value = (
                        self._extract_value_from_record(record, size_field)
                        if size_field
                        else 5
                    )
                    group = (
                    ...
                    point = {"x": x_value, "y": y_value, "group": group}
    ...
    def _extract_value_from_record(
        self, record: Dict[str, Any], field: Optional[str]
    ) -> Any:
        ...
        for part in parts:
            if isinstance(value, dict) and part in value:
                ...
                and value.get(part) is not None
            ):
