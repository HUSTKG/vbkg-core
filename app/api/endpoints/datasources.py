...
from fastapi import APIRouter, Depends, HTTPException, status as HttpStatus, File, UploadFile, Form, Query, Path
...
@router.post("/", response_model=ApiResponse[DataSource], status_code=HttpStatus.HTTP_201_CREATED)
...
@router.get("/", response_model=PaginatedResponse[DataSource])
...
@router.post("/{datasource_id}/upload", response_model=ApiResponse[FileUpload], status_code=HttpStatus.HTTP_201_CREATED)
