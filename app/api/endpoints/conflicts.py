...
from fastapi import APIRouter, Depends, HTTPException, status as HttpStatus, Path, Query
...
async def list_conflicts(
    status: Optional[str] = Query(None, description="Filter by conflict status (pending/resolved/ignored)"),
    ...
    return PaginatedResponse(
        ...
        meta={
            "skip": skip,
            "limit": limit,
            "total": len(conflicts)  # In a real implementation, you'd want to get the total count
        },
...
async def detect_conflicts(
    ...
    batch_size: int = Query(100, ge=1, le=1000, description="Number of entities to process"),
...
async def find_potential_conflicts(
    ...
    threshold: float = Query(0.8, ge=0.0, le=1.0, description="Similarity threshold"),
    max_conflicts: int = Query(5, ge=1, le=100, description="Maximum number of conflicts to return"),
    current_user: Dict[str, Any] = Depends(check_read_permission)
...
async def merge_entity_pair(
    ...
    keep_id: Optional[UUID] = Query(None, description="ID of the entity to keep (optional)"),
